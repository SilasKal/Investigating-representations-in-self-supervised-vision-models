...
def _load_pretrained_weights(model, arch: str):
    """Load torchvision ImageNet-V1 weights into our custom ResNet (skips fc layer)."""
    import torchvision.models as tv_models
    if arch == "resnet18":
        tv_model = tv_models.resnet18(weights=tv_models.ResNet18_Weights.IMAGENET1K_V1)
    elif arch == "resnet50":
        tv_model = tv_models.resnet50(weights=tv_models.ResNet50_Weights.IMAGENET1K_V1)
    else:
        raise ValueError(f"No pretrained weights available for {arch}")
    # Our ResNet has no fc layer, so skip fc.* keys
    pretrained_sd = {k: v for k, v in tv_model.state_dict().items() if not k.startswith("fc.")}
    missing, unexpected = model.load_state_dict(pretrained_sd, strict=False)
    print(f"[pretrained] loaded {len(pretrained_sd)} keys, missing={len(missing)}, unexpected={len(unexpected)}")
    return model
...
def get_network(args):
    ...
    if getattr(args, "pretrained", False):
        model = _load_pretrained_weights(model, args.model)
